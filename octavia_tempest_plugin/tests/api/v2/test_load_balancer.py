...
class LoadBalancerAPITest(test_base.LoadBalancerBaseTest):
    ...
    @decorators.idempotent_id('4e3c5ad2-8f4e-4444-befc-ed2e91605bf7')
    @testtools.skipUnless(CONF.load_balancer.test_with_ipv6,
                          'IPv6 testing is disabled')
    def test_loadbalancer_ipv4_create_additional_vips(self):
        # There's an issue with the network noop driver: there's no safe method
        # to use 2 subnets from the same network (which is required by
        # multivip), it works when using tempest with one thread but it
        # triggers issues when using multiple thread.
        # https://bugs.launchpad.net/octavia/+bug/2036383
        if CONF.load_balancer.test_with_noop:
            raise self.skipException(
                'Additional VIPs feature is not supported with network noop '
                'driver')
        if not self.mem_lb_client.is_version_supported(
                self.api_version, '2.26'):
            raise self.skipException(
                'Additional VIPs feature requires '
                'Octavia API version 2.26 or newer.')
        additional_vips = [
            {const.SUBNET_ID: self.lb_member_vip_subnet2[const.ID]},
            {const.SUBNET_ID: self.lb_member_vip_ipv6_subnet2[const.ID]}]
