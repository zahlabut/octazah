...
class LoadBalancerBaseTest(validators.ValidatorsMixin,
    ...
    def resource_setup(cls, extra_subnets=False):
        ...
        if CONF.load_balancer.test_with_noop:
            ...
            if extra_subnets:
                cls.lb_member_vip_subnet2 = {'id': uuidutils.generate_uuid()}
                cls.lb_member_1_subnet2 = {'id': uuidutils.generate_uuid()}
            if CONF.load_balancer.test_with_ipv6:
                cls.lb_member_vip_ipv6_net = {'id': uuidutils.generate_uuid()}
                cls.lb_member_vip_ipv6_subnet = {'id':
                                                 uuidutils.generate_uuid()}
                if extra_subnets:
                    cls.lb_member_vip_ipv6_subnet2 = {
            ...
            cls.lb_member_vip_subnet2 = override_subnet
            cls.lb_member_1_net = override_network
            cls.lb_member_1_subnet = override_subnet
            cls.lb_member_1_subnet2 = override_subnet
            ...
            if (CONF.load_balancer.test_with_ipv6 and
                ...
                cls.lb_member_vip_ipv6_subnet2 = override_ipv6_subnet
                ...
                cls.lb_member_vip_ipv6_subnet2 = None
                cls.lb_member_1_ipv6_subnet = None
                cls.lb_member_2_ipv6_subnet = None
        else:
            cls._create_networks(extra_subnets)
        ...
        if extra_subnets and cls.lb_member_vip_subnet2:
            LOG.debug('Octavia Setup: lb_member_vip_subnet2 = %s',
                      cls.lb_member_vip_subnet2[const.ID])
        if cls.lb_member_1_subnet:
            LOG.debug('Octavia Setup: lb_member_1_subnet = %s',
                      cls.lb_member_1_subnet[const.ID])
        if extra_subnets and cls.lb_member_1_subnet2:
            LOG.debug('Octavia Setup: lb_member_1_subnet2 = %s',
                      cls.lb_member_1_subnet2[const.ID])
        ...
        if CONF.load_balancer.test_with_ipv6:
            ...
            if extra_subnets and cls.lb_member_vip_ipv6_subnet2:
                LOG.debug('Octavia Setup: lb_member_vip_ipv6_subnet2 = %s',
                          cls.lb_member_vip_ipv6_subnet2[const.ID])
    ...
    @classmethod
    def _create_networks(cls, extra_subnets=False):
        ...
        The following are expected to be defined and available to the tests:
            ...
            cls.lb_member_vip_subnet2 (optional)
            cls.lb_member_vip_ipv6_subnet2 (optional)
            cls.lb_member_1_net
            cls.lb_member_1_subnet
            cls.lb_member_1_subnet2 (optional)
        ...
        # Create optional 2nd tenant VIP subnet
        if extra_subnets:
            subnet_kwargs = {
                'name': data_utils.rand_name("lb_member_vip_subnet2"),
                'network_id': cls.lb_member_vip_net['id'],
                'cidr': CONF.load_balancer.vip_subnet2_cidr,
                'ip_version': 4}
            result = cls.lb_mem_subnet_client.create_subnet(**subnet_kwargs)
            cls.lb_member_vip_subnet2 = result['subnet']
            cls.addClassResourceCleanup(
                waiters.wait_for_not_found,
                cls._logging_delete_subnet,
                cls.lb_mem_subnet_client.show_subnet,
                cls.lb_member_vip_subnet2['id'])
        else:
            cls.lb_member_vip_subnet2 = None

        LOG.info('lb_member_vip_subnet2: %s',
                 cls.lb_member_vip_subnet2)
        ...
        if CONF.load_balancer.test_with_ipv6:
            ...
            if extra_subnets:
                subnet_kwargs = {
                    'name': data_utils.rand_name("lb_member_vip_ipv6_subnet2"),
                    'network_id': cls.lb_member_vip_net['id'],
                    'ip_version': 6}

                if subnetpool_name:
                    subnetpool = (
                        cls.os_admin_subnetpools_client.list_subnetpools(
                            name=subnetpool_name)['subnetpools'])
                    if len(subnetpool) == 1:
                        subnetpool = subnetpool[0]
                        subnet_kwargs['subnetpool_id'] = subnetpool['id']
                ...
                if 'subnetpool_id' not in subnet_kwargs:
                    subnet_kwargs['cidr'] = (
                        CONF.load_balancer.vip_ipv6_subnet2_cidr)

                result = cls.lb_mem_subnet_client.create_subnet(
                    **subnet_kwargs)
                cls.lb_member_vip_ipv6_subnet2 = result['subnet']
                cls.addClassResourceCleanup(
                    waiters.wait_for_not_found,
                    cls._logging_delete_subnet,
                    cls.lb_mem_subnet_client.show_subnet,
                    cls.lb_member_vip_ipv6_subnet2['id'])
            else:
                cls.lb_member_vip_ipv6_subnet2 = None

            LOG.info('lb_member_vip_ipv6_subnet2: %s',
                     cls.lb_member_vip_ipv6_subnet2)
        ...
        if extra_subnets:
            # Create 2nd tenant member 1 subnet
            subnet_kwargs = {
                'name': data_utils.rand_name("lb_member_1_subnet2"),
                'network_id': cls.lb_member_1_net['id'],
                'cidr': CONF.load_balancer.member_1_ipv4_subnet2_cidr,
                'ip_version': 4}
            result = cls.lb_mem_subnet_client.create_subnet(**subnet_kwargs)
            cls.lb_member_1_subnet2 = result['subnet']
            cls.addClassResourceCleanup(
                waiters.wait_for_not_found,
                cls._logging_delete_subnet,
                cls.lb_mem_subnet_client.show_subnet,
                cls.lb_member_1_subnet2['id'])
        else:
            cls.lb_member_1_subnet2 = None
        LOG.info('lb_member_1_subnet2: %s', cls.lb_member_1_subnet2)
...
class LoadBalancerBaseTestWithCompute(LoadBalancerBaseTest):
    ...
    def _create_networks(cls, extra_subnets=False):
        ...
        if extra_subnets:
            ...
            cls.lb_mem_routers_client.add_router_interface(
                cls.lb_member_router['id'],
                subnet_id=cls.lb_member_vip_subnet2['id'])
            cls.addClassResourceCleanup(
                waiters.wait_for_not_found,
                cls.lb_mem_routers_client.remove_router_interface,
                cls.lb_mem_routers_client.remove_router_interface,
        ...
        # Add IPv6 VIP subnets if testing with IPv6
        if CONF.load_balancer.test_with_ipv6:
            # Add IPv6 VIP subnet to router
            cls.lb_mem_routers_client.add_router_interface(
                cls.lb_member_router['id'],
            ...
            if extra_subnets:
                # Add IPv6 VIP subnet2 to router
                cls.lb_mem_routers_client.add_router_interface(
                    cls.lb_member_router['id'],
                    subnet_id=cls.lb_member_vip_ipv6_subnet2['id'])
                cls.addClassResourceCleanup(
                    waiters.wait_for_not_found,
                    cls.lb_mem_routers_client.remove_router_interface,
                    cls.lb_mem_routers_client.remove_router_interface,
        ...
        if extra_subnets:
            # Add 2nd member subnet 1 to router
            cls.lb_mem_routers_client.add_router_interface(
                cls.lb_member_router['id'],
                subnet_id=cls.lb_member_1_subnet2['id'])
            cls.addClassResourceCleanup(
                waiters.wait_for_not_found,
                cls.lb_mem_routers_client.remove_router_interface,
                cls.lb_mem_routers_client.remove_router_interface,
        ...
        # Add IPv6 member subnets if testing with IPv6
        if CONF.load_balancer.test_with_ipv6:
            # Add member subnet ipv6 1 to router
            cls.lb_mem_routers_client.add_router_interface(
                cls.lb_member_router['id'],
                subnet_id=cls.lb_member_1_ipv6_subnet['id'])
            cls.addClassResourceCleanup(
                waiters.wait_for_not_found,
                cls.lb_mem_routers_client.remove_router_interface,
                cls.lb_mem_routers_client.remove_router_interface,
            ...
            # Add member subnet ipv6 2 to router
            cls.lb_mem_routers_client.add_router_interface(
                cls.lb_member_router['id'],
                subnet_id=cls.lb_member_2_ipv6_subnet['id'])
            cls.addClassResourceCleanup(
                waiters.wait_for_not_found,
                cls.lb_mem_routers_client.remove_router_interface,
                cls.lb_mem_routers_client.remove_router_interface,
