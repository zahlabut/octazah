...
class MemberBaseScenarioTest(test_base.LoadBalancerBaseTest):

    @classmethod
    def resource_setup(cls, extra_subnets=False):
        """Setup shared resources needed by the tests."""
        super(MemberBaseScenarioTest, cls).resource_setup(
            extra_subnets)
...
class MemberScenarioTest(MemberBaseScenarioTest):
...
class MemberScenarioTestMultipleSubnets(MemberBaseScenarioTest):
    @classmethod
    def resource_setup(cls):
        super(MemberScenarioTestMultipleSubnets, cls).resource_setup(
            extra_subnets=True)

        if not cls.mem_lb_client.is_version_supported(
                cls.api_version, '2.24'):
            # the fix for the multiple subnets on member is available in
            # stable/wallaby
            raise cls.skipException(
                'Multiple Subnet tests require Octavia API version 2.24 '
                'or newer.')

    def _member_add(self, pool_id, subnet_id, ip_address):
        member_name = data_utils.rand_name("lb_member_member1-create")
        member_kwargs = {
            const.NAME: member_name,
            const.ADMIN_STATE_UP: True,
    ...
    def _member_delete(self, pool_id, member_id):
        self.mem_member_client.delete_member(
            member_id,
            pool_id=pool_id)

        waiters.wait_for_deleted_status_or_not_found(
            self.mem_member_client.show_member, member_id,
            const.PROVISIONING_STATUS,
            CONF.load_balancer.check_interval,
            CONF.load_balancer.check_timeout,
            pool_id=pool_id)
    ...
    def _get_ports(self, compute_id):
        return self.lb_mem_ports_client.list_ports(
            device_id=compute_id)['ports']

    def _get_subnet_ids_from_ports(self, ports):
        return [
            fixed_ip[const.SUBNET_ID]
            for port in ports
            for fixed_ip in port['fixed_ips']
        ]

    @decorators.idempotent_id('d8f45537-0538-43dc-9dc3-8e753b8e6a75')
    def test_member_network_with_multiple_subnets(self):
        pool_id = self._listener_pool_create(
            const.HTTP, const.HTTP, const.LB_ALGORITHM_ROUND_ROBIN)
        ...
        amphora = amphorae[0]
        compute_id = amphora[const.COMPUTE_ID]

        # Get ports and subnets
        # only VIP subnet/network is plugged
        ports = self._get_ports(compute_id)
        port_subnet_ids = self._get_subnet_ids_from_ports(ports)

        self.assertEqual(1, len(ports))
        self.assertEqual(1, len(port_subnet_ids))
        self.assertIn(self.lb_member_vip_subnet[const.ID],
                      port_subnet_ids)

        # Create a member on the 2nd VIP subnet
        member1 = self._member_add(
            pool_id,
            self.lb_member_vip_subnet2[const.ID],
            '10.1.0.1')

        # Get ports and subnets
        # the VIP network is plugged with 2 subnets
        ports = self._get_ports(compute_id)
        port_subnet_ids = self._get_subnet_ids_from_ports(ports)
        ...
        self.assertIn(self.lb_member_vip_subnet[const.ID],
                      port_subnet_ids)
        self.assertIn(self.lb_member_vip_subnet2[const.ID],
                      port_subnet_ids)

        # Create a member on the member_1 subnet
        member2 = self._member_add(
            pool_id,
            self.lb_member_1_subnet[const.ID],
            '10.2.0.1')

        # Get ports and subnets
        # the VIP network is plugged with 2 subnets
        # member_1 subnet has been added
        ports = self._get_ports(compute_id)
        port_subnet_ids = self._get_subnet_ids_from_ports(ports)
        ...
        self.assertIn(self.lb_member_vip_subnet2[const.ID],
                      port_subnet_ids)
        self.assertIn(self.lb_member_1_subnet[const.ID],
                      port_subnet_ids)

        # Create a member on member_2 subnet
        member3 = self._member_add(
            pool_id,
            self.lb_member_2_subnet[const.ID],
            '10.3.0.1')

        # Get ports and subnets
        # the VIP network is plugged with 2 subnets
        # member_1 subnet is plugged
        # member_2 subnet has been added
        ports = self._get_ports(compute_id)
        port_subnet_ids = self._get_subnet_ids_from_ports(ports)
        ...
        self.assertIn(self.lb_member_1_subnet[const.ID],
                      port_subnet_ids)
        self.assertIn(self.lb_member_2_subnet[const.ID],
                      port_subnet_ids)

        # Create a member on the 2nd member_1 subnet
        member4 = self._member_add(
            pool_id,
            self.lb_member_1_subnet2[const.ID],
            '10.4.0.1')

        # Get ports and subnets
        # the VIP network is plugged with 2 subnets
        # 2 subnets on member_1 are plugged
        # member_2 subnet is plugged
        ports = self._get_ports(compute_id)
        port_subnet_ids = self._get_subnet_ids_from_ports(ports)
        ...
        self.assertIn(self.lb_member_2_subnet[const.ID],
                      port_subnet_ids)
        self.assertIn(self.lb_member_1_subnet2[const.ID],
                      port_subnet_ids)

        # Delete the member plugged on the 2nd VIP subnet
        self._member_delete(pool_id, member1[const.ID])

        # Get ports and subnets
        # the VIP network is still plugged with 1 subnet
        # 2 subnets on member_1 are plugged
        # member_2 subnet is plugged
        ports = self._get_ports(compute_id)
        port_subnet_ids = self._get_subnet_ids_from_ports(ports)
        ...
        self.assertIn(self.lb_member_vip_subnet[const.ID],
                      port_subnet_ids)
        self.assertNotIn(self.lb_member_vip_subnet2[const.ID],
                         port_subnet_ids)
        self.assertIn(self.lb_member_1_subnet[const.ID],
                      port_subnet_ids)
        self.assertIn(self.lb_member_2_subnet[const.ID],
        ...
        # Delete the member plugged on member_1 subnet
        self._member_delete(pool_id, member2[const.ID])

        # Get ports and subnets
        # the VIP network is still plugged with 1 subnet
        # 2nd member_1 subnet is plugged
        # member_2 subnet is plugged
        ports = self._get_ports(compute_id)
        port_subnet_ids = self._get_subnet_ids_from_ports(ports)
        ...
        self.assertNotIn(self.lb_member_vip_subnet2[const.ID],
                         port_subnet_ids)
        self.assertNotIn(self.lb_member_1_subnet[const.ID],
                         port_subnet_ids)
        self.assertIn(self.lb_member_2_subnet[const.ID],
                      port_subnet_ids)
        self.assertIn(self.lb_member_1_subnet2[const.ID],
        ...
        # Delete the member plugged on member_2 subnet
        self._member_delete(pool_id, member3[const.ID])
        ...
        self.assertNotIn(self.lb_member_1_subnet[const.ID],
                         port_subnet_ids)
        self.assertNotIn(self.lb_member_2_subnet[const.ID],
                         port_subnet_ids)
        self.assertIn(self.lb_member_1_subnet2[const.ID],
                      port_subnet_ids)

        # Delete the member plugged on 2nd member_1 subnet
        self._member_delete(pool_id, member4[const.ID])
        ...
        self.assertNotIn(self.lb_member_2_subnet[const.ID],
                         port_subnet_ids)
        self.assertNotIn(self.lb_member_1_subnet2[const.ID],
                         port_subnet_ids)
